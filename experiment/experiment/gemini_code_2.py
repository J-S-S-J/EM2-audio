...
# FACE_DURATION = 0.500  # 500ms
# MASK_DURATION = 0.250  # 250ms
...
def run_trial(win, base_dir, trial_info, trial_handler, stimuli, rt_clock):
    ...
    # Execute Timeline
    babble_sound.play(sync=False)  # Asynchronous
    mask1_sound.play(sync=True)    # Pauses script
    
    if prime_sound:
        prime_sound.play(sync=True) # Pauses script (if it exists)
        
    mask2_sound.play(sync=True)    # Pauses script
    ...
    # --- 7. Out Sound ---
    stimuli['out_sound'].play(sync=False) # Plays during next trial's fixation
